...
export const SecondaryButton = ({ className, children, onClick }: Props) => {
  return (
    <button
      onClick={onClick}
      className={`flex-shrink-0 rounded-full border-0 bg-zinc-50 py-3 font-michroma text-base text-zinc-800 shadow-2xl transition-all focus:outline-none sm:mt-0 md:hover:bg-zinc-200 ${className}`}
    >
      {children}
    </button>
  );
